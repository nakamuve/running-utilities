<script>
	...
	const calculatorComponentMap = {
		...
		'/eighty-twenty-calculator': () =>
			import('../lib/components/calculators/EightyTwentyCalculator.svelte'),
		'/friel-calculator': () => import('../lib/components/calculators/FrielCalculator.svelte'),
		'/km-effort-calculator': () =>
			import('../lib/components/calculators/KmEffortCalculator.svelte'),
		'/gpx-elevation-calculator': () =>
			import('../lib/components/calculators/GpxElevationCalculator.svelte')
	};
...
<svelte:head>
	<title>Running Calculator Utilities - Nakamuve</title>
</svelte:head>

<div class="container mx-auto px-4 py-8">
	<h1 class="mb-6 text-2xl font-bold">Running Calculator Utilities</h1>

	<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
		{#each calculatorCategories as category}
			<div class="rounded-lg bg-white p-6 shadow-md">
				<h2 class="mb-4 text-lg font-semibold">{category.title}</h2>
				<ul class="space-y-2">
