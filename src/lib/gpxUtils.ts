...
export function parseGPXString(gpxString: string): {
	...
	try {
		...
		let trkptNodes = xmlDoc.evaluate(
			'//trkpt | //gpx:trkpt',
			xmlDoc,
		...
		throw new Error(
			error instanceof Error
				? error.message
				: "Failed to parse GPX file. Please ensure it's a valid GPX format."
		);
	}
}
...
export function smoothElevationData(
	...
	for (let i = 0; i < trackPoints.length; i++) {
		...
		const avgElevation =
			windowElevations.reduce((sum, ele) => sum + ele, 0) / windowElevations.length;
...
export function calculateElevationGainForSegments(
	...
	for (let i = 1; i < pointsToUse.length; i++) {
		...
		if (totalDistance >= currentSegment * segmentDistanceMeters || i === pointsToUse.length - 1) {
			...
			const segmentElevationChanges = calculateTotalElevationChanges(
				segmentPoints,
				minElevationDiff
			);
	...
	const avgElevationGainPerKm =
		segmentResults.length > 0 ? totalElevationGain / segmentResults.length : 0;
