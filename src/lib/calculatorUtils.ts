...
export function mcmillanCalculate(distance: string, raceTimeSeconds: number) {
	...
	const distanceFactors: Record<string, { seconds: number; meters: number }> = {
		...
		'15k': { seconds: (15000 / 400) * 1.1, meters: 15000 },
...
export function humphreyCalculate(distance: string, raceTimeSeconds: number) {
	...
	const trainingPaces = {
		...
		strength_workouts: {
			pace: formatPace(base5kPacePerKm * 1.1)
		},
		half_marathon_tempo: {
...
export function eightyTwentyCalculate(distance: string, raceTimeSeconds: number) {
	...
	const paceZones = {
		zone1: {
			...
			max: formatPace(vt2Pace * 1.2), // 80% of VT2 intensity
			description: 'Recovery/Low-Intensity'
		},
		zone2: {
			// Foundation/Endurance
			min: formatPace(vt2Pace * 1.2), // 80% of VT2 intensity
		...
		zoneZ: {
			...
			max: formatPace(vt2Pace * 0.7), // Arbitrary faster limit
 ...
 */
export function frielCalculate(
	lthr: number | null = null,
	ftp: number | null = null,
	ltPaceSeconds: number | null = null
) {
	const result: any = {};
	...
	if (ltPaceSeconds) {
		const paceZones = {
			zone1: {
				// Active Recovery
				min: ltPaceSeconds * 1.3,
				max: ltPaceSeconds * 1.2,
				description: 'Active Recovery'
			},
			zone2: {
				// Endurance
				min: ltPaceSeconds * 1.2,
				max: ltPaceSeconds * 1.1,
				description: 'Endurance'
			},
			zone3: {
				// Tempo
				min: ltPaceSeconds * 1.1,
			...
			zone4: {
				...
				max: ltPaceSeconds * 1.0,
				description: 'Lactate Threshold'
			},
			zone5a: {
				// VO2max
				min: ltPaceSeconds * 1.0,
			...
			zone5b: {
				...
				max: ltPaceSeconds * 0.9,
				description: 'Anaerobic Capacity'
			},
			zone5c: {
				// Neuromuscular Power
				min: ltPaceSeconds * 0.9,
				max: ltPaceSeconds * 0.8,
				description: 'Neuromuscular Power'
			}
		};
